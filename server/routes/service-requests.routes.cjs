...
router.post('/:id/convert', asyncRoute(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const order = await tx.order.create({ data: { clientId: client.id, serviceType: request.serviceType === 'Otro' ? request.customServiceType : request.serviceType, originAddress: request.originAddress, destinationAddress: request.destinationAddress, cargoDescription: request.cargoDescription, cargoWeightKg: request.cargoWeightKg, requestedDate: request.requestedDate, vehicleMake: request.vehicleMake, vehicleModel: request.vehicleModel, vehicleYear: request.vehicleYear, vehicleVin: request.vehicleVin, status: 'REQUESTED', paymentStatus: 'PENDING', trackingNumber, trackingCode: trackingNumber, trackingToken } });
